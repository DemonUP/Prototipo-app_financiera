...
export default function BranchPathScreen({ route, navigation }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#f5f7fb' }} contentContainerStyle={{ padding: 16, alignItems: 'center' }}>
      ...
      <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 14, width: '100%', maxWidth: 860, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 2 }}>
      ...
      <View style={{ width: '100%', maxWidth: 860, marginTop: 16, backgroundColor: '#ffffffaa', borderRadius: 16, paddingVertical: 24 }}>
        {data.map((item, idx) => (
          <LevelNode key={item.id} index={idx} item={item} isLocked={item.locked} />
        ))}
      </View>
    </ScrollView>
  );
